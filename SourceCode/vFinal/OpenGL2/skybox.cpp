...
const char* hdrsource = "./HDR/107_hdrmaps_com_free.hdr";
//const char* hdrsource = "./HDR/satara_night_4k.hdr";



// pbr: set up projection and view matrices for capturing data onto the 6 cubemap face directions
// ----------------------------------------------------------------------------------------------
//之前在帧缓冲和点阴影教程中讨论过的代码细节：面向立方体六个面设置六个不同的视图矩阵，给定投影矩阵的 fov 为 90 度以捕捉整个面，并渲染立方体六次，将结果存储在浮点帧缓冲中：
...
void StbMapHDR(unsigned int& hdrTexture)
{
    ...
    float* data = stbi_loadf(hdrsource, &width, &height, &nrComponents, 0);
    if (data)
    {
...
void CeateIrradiance(unsigned int& irradianceMap, unsigned int& captureFBO, unsigned int& captureRBO)
{
    ...
    /*由于辐照度图对所有周围的辐射值取了平均值，
        因此它丢失了大部分高频细节，所以我们可以以较低的分辨率（32x32）存储，
        并让 OpenGL 的线性滤波完成大部分工作。接下来，我们将捕捉到的帧缓冲图像缩放到新的分辨率：*/
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
...
)
{
    ...
    //我们使用的是 Epic Games 推荐的16位精度浮点格式。将环绕模式设置为 GL_CLAMP_TO_EDGE 以防止边缘采样的伪像。
    //    然后，我们复用同一个帧缓冲区对象， 
    //    并在 NDC(译注：Normalized Device Coordinates) 屏幕空间四边形上运行此着色器：
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, 1024, 1024);
